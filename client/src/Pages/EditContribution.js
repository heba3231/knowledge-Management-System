// client/src/components/pages/EditContribution.js
...
const EditContribution = () => {
  ...
  useEffect(() => {
    const fetchContribution = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/contributions/${id}`, {
        ...
        setError('فشل في تحميل المبادرة');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/contributions/${id}`, data, {
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.pageTitle}>تعديل المبادرة</h1>
