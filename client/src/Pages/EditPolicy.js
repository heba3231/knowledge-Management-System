// client/src/Pages/EditPolicy.js
    ...
    const fetchDocument = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setFormData({
            title: doc.title,
            subCategory: doc.subCategory || 'HR',
            department: doc.department || 'General',
            description: doc.description || '',
          });
        } else {
          setError('فشل في تحميل بيانات السياسة');
        }
      } catch (err) {
        setError('تعذر تحميل السياسة. حاول مرة أخرى.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'فشل في تحديث السياسة');
      }
    } catch (err) {
      console.error('Update error:', err);
      setError('فشل تحديث السياسة. حاول مرة أخرى.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.pageTitle}>✏️ تعديل السياسة</h1>
        <p style={styles.pageSubtitle}>قم بتحديث بيانات السياسة</p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <div style={styles.buttons}>
          ...
          >
            {submitting ? 'جاري التحديث...' : '💾 تحديث السياسة'}
          </button>
          <button
            type="button"
