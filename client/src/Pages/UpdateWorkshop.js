...
const UpdateWorkshop = () => {
  ...
  // جلب بيانات الورشة الحالية (بدون توكن لأن الـ API عام)
  useEffect(() => {
    const fetchWorkshop = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setError('Failed to load workshop data: ' + (response.data.message || 'Unknown error'));
        }
      } catch (err) {
        console.error('Error fetching workshop:', err);
        if (err.response) {
          // الخادم استجاب بحالة خطأ (404, 500, إلخ)
  ...
  // باقي الدوال (handleChange, handleRemove..., handleSubmit) كما هي ...
  ...
  if (error) {
    return (
      <div style={styles.container}>
        ...
        <button onClick={() => navigate('/workshops')} style={styles.backBtn}>
          ← Back to Workshops
        </button>
      </div>
    );
  ...
  // إذا لم توجد بيانات (حالة نادرة)
  if (!formData.title && !formData.existingImages.length && !loading) {
    return (
      <div style={styles.container}>
        <div style={styles.errorAlert}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '8px', flexShrink: 0 }}>
  ...
  return (
    <div style={styles.container}>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        {/* إضافة فيديوهات جديدة مع عرض الأسماء وزر إزالة */}
        <div style={styles.field}>
